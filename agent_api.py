    ...
    for thread_id in thread_ids:
        ...
        try:
            ...
            if (state_snapshot and 
                state_snapshot.next and 
                "hitl_decision" in state_snapshot.next):
                ...
                invoice_id = (invoice_data.get("invoice_id") or 
                             invoice_payload.get("invoice_id") or 
                             f"INV-{thread_id[-8:]}")
                
                # Try to get vendor name from multiple sources
                vendor_name = (state.get("normalized_vendor_name") or
                              invoice_data.get("vendor_name") or
                              invoice_payload.get("vendor_name") or
                              invoice_data.get("vendor") or
                              "Tech Solutions Inc")
                
                # Try to get amount from multiple sources
                amount = (invoice_data.get("amount") or
                         invoice_payload.get("amount") or
                         invoice_data.get("total") or
                         1250.75)
    ...
    try:
        ...
        if invoice_data:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid JSON in invoice_data: {str(e)}"
                )
        ...
        if not invoice_payload.get("invoice_id"):
            invoice_payload["invoice_id"] = f"INV-{datetime.now().strftime('%Y%m%d%H%M%S')}"
...
async def submit_decision(decision: AgentDecision):
    ...
    try:
        ...
        return {
            ...
            "next_stage": "RECONCILE" if decision.decision == "ACCEPT" else "MANUAL_HANDOFF",
            "status": status,
...
if __name__ == "__main__":
    ...
    print("="*70)
    logger.info("\n📋 Agent Endpoints:")
    logger.info("  POST   /api/agent/invoke          - Invoke agent (upload image/data)")
